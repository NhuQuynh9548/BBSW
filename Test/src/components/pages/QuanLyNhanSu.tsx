...
import { salaryConfigService, SalaryConfig } from '../../services/salaryConfigService';
...
const DEFAULT_COLUMNS: ColumnConfig[] = [
  ...
  { id: 'actualSalary', label: 'Lương thực nhận', field: 'actualSalary', align: 'right', visible: false },
  { id: 'contractType', label: 'Hợp đồng', field: 'contractType', align: 'center', visible: false },
  ...
  { id: 'contractStartDate', label: 'Ngày Bắt Đầu', field: 'contractStartDate', align: 'center', visible: false },
  { id: 'contractEndDate', label: 'Ngày Kết Thúc', field: 'contractEndDate', align: 'center', visible: false },
...
export function QuanLyNhanSu() {
  ...
  const [formData, setFormData] = useState({
    ...
    address: '',
    actualSalary: 0,
    contractType: 'chinh-thuc' as 'ctv' | 'chinh-thuc' | 'thuc-tap',
    contractStartDate: '',
    contractEndDate: '',
  });
  ...
  useEffect(() => {
    const fetchMasterData = async () => {
      try {
        ...
        setLevelList(levels.sort((a: any, b: any) => a.order - b.order)); // Sort by order
      } catch (err) {
  ...
  const handleEdit = (employee: Employee) => {
    ...
    setFormData({
      ...employee,
      actualSalary: employee.actualSalary ?? 0,
      contractType: employee.contractType ?? 'chinh-thuc',
  ...
  const renderCell = (column: ColumnConfig, employee: Employee) => {
    ...
    switch (column.id) {
        ...
        return (
          <td key={column.id} className={`py-4 px-6 whitespace-nowrap ${alignClass}`}>
            <span className="font-bold text-blue-700">
              {new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(employee.actualSalary || 0)}
            </span>
          </td>
        );
      ...
      case 'contractStartDate':
        return (
          <td key={column.id} className={`py-4 px-6 whitespace-nowrap ${alignClass}`}>
            <span className="text-sm text-gray-600">{employee.contractStartDate}</span>
          </td>
        );
      ...
      case 'contractEndDate':
        return (
          <td key={column.id} className={`py-4 px-6 whitespace-nowrap ${alignClass}`}>
            <span className="text-sm text-gray-600">{employee.contractEndDate || '-'}</span>
          </td>
        );
    ...
    <div className="p-8">
      ...
      {!loading && (
        <>
                ...
                <div className="overflow-y-auto flex-1 px-6 py-6">
                  <form onSubmit={handleSubmit} id="employee-form">
                    <div className="space-y-5">
                      ...
                      {/* Row 8: Lương thực nhận & Loại hợp đồng */}
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <label className="block text-sm font-semibold text-gray-700 mb-2 uppercase flex items-center gap-2">
                          ...
                          <div className="relative">
                            ...
                            <input
                              ...
                              disabled={isReadOnly}
                              className="w-full pl-10 pr-4 py-2.5 bg-gray-50 border border-gray-200 rounded-lg focus:ring-2 focus:ring-[#004aad] focus:border-transparent focus:bg-white transition-all disabled:bg-gray-100 disabled:cursor-not-allowed font-bold text-blue-700"
                            />
                          </div>
                        </div>
                        <div>
                      ...
                      {/* Row 9: Ngày bắt đầu & Ngày kết thúc */}
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <label className="block text-sm font-semibold text-gray-700 mb-2 uppercase flex items-center gap-2">
                            <Calendar className="w-4 h-4" />
                          ...
                          <input
                            type="date"
                            value={formData.contractStartDate ? formData.contractStartDate.split('/').reverse().join('-') : ''}
                            onChange={(e) => {
                        ...
                        <div>
                          ...
                          <input
                            type="date"
                            value={formData.contractEndDate ? formData.contractEndDate.split('/').reverse().join('-') : ''}
                            onChange={(e) => {
                              const date = e.target.value ? e.target.value.split('-').reverse().join('/') : '';
                              setFormData({ ...formData, contractEndDate: date });
                            }}
                            disabled={isReadOnly}
                            className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-lg focus:ring-2 focus:ring-[#004aad] focus:border-transparent focus:bg-white transition-all disabled:bg-gray-100 disabled:cursor-not-allowed"
                          />
          ...
          {showSalaryDetails && selectedEmployee && salaryConfig && (
            <ChiTietLuongNhanVien
              ...
              onUpdate={(empId, newSalary) => {
                ...
                // Sync selected employee to update the modal content
                setSelectedEmployee(prev => prev && prev.id === empId ? { ...prev, actualSalary: newSalary } : prev);
                // Refresh list from server for full synchronization
                fetchEmployees();
              }}
              onClose={() => {
