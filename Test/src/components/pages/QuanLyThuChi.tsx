...
export function QuanLyThuChi() {
  ...
  // Helper: check if an attachment is an image (by fileType or filename extension)
  const isImageFile = (file: { fileType: string; fileName: string }) => {
    ...
    return [
      ...
      'gif',
      'webp',
      'heic', 'heif', 'heicv', 'heics', 'heivs',
      'tiff', 'tif',
      'avif', 'jxl',
      'svg', 'svgz',
      'ico', 'cur',
      'dng', 'cr2', 'cr3', 'nef', 'nrw', 'arw', 'srf', 'sr2', 'rw2', 'orf', 'raf', 'raw',
      'psd', 'psb',
    ].includes(ext || '');
  };

  // Helper: check if an attachment is a video
  const isVideoFile = (file: { fileType: string; fileName: string }) => {
    if (file.fileType && file.fileType.startsWith('video/')) return true;
    const ext = file.fileName?.split('.').pop()?.toLowerCase();
    return [
      'mp4', 'm4v', 'mov', 'avi', 'mkv', 'webm', '3gp', '3g2',
      'ogv', 'mpeg', 'mpg', 'wmv', 'flv', 'hevc', 'h264', 'h265',
      'ts', 'mts', 'm2ts', 'mxf', 'vob', 'rm', 'rmvb', 'asf',
      'divx', 'xvid', 'f4v', 'm2v', 'qt',
    ].includes(ext || '');
  };
                          ...
                          {formData.objectType === 'STUDENT' || formData.objectType === 'OTHER' ? (
                            <div className="flex gap-2">
                              <input
                                ...
                                className="w-full px-4 py-2 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none transition-all flex-1"
                              />
                              {formData.objectType === 'OTHER' && modalMode !== 'view' && (
                                <button
                                  type="button"
                                  onClick={() => setShowPartnerModal(true)}
                                  className="p-2 bg-blue-50 text-blue-600 border border-blue-200 rounded-lg hover:bg-blue-100 transition-colors flex items-center gap-1 text-xs font-semibold px-3"
                                  title="Tạo đối tác vĩnh viễn"
                                >
                                  <Plus className="w-4 h-4" /> Tạo Đối Tác
                                </button>
                              )}
                            </div>
                          ) : (
                            <div className="flex gap-2">
                              ...
                              {formData.objectType === 'PARTNER' && modalMode !== 'view' && (
                                <button
                                  type="button"
                                  onClick={() => setShowPartnerModal(true)}
                            ...
                            {formData.attachments.map((file, idx) => (
                              <div key={file.id || idx} className="relative group border rounded-lg overflow-hidden bg-gray-50 flex flex-col">
                                <div className="aspect-video bg-gray-200 flex items-center justify-center overflow-hidden">
                                  ...
                                  <div className={`flex flex-col items-center gap-1 ${isImageFile(file) || isVideoFile(file) ? 'hidden' : ''}`}>
                                    <FileText className="w-10 h-10 text-gray-400" />
                                    <span className="text-[10px] text-gray-400 font-medium">{file.fileName.split('.').pop()?.toUpperCase() || 'FILE'}</span>
                                  </div>
                                  {isVideoFile(file) && (
                      ...
                      {modalMode !== 'view' && (
                        <div>
                          ...
                          <input
                            type="file"
                            multiple
                            accept="image/*,video/*,.jpg,.jpeg,.jpe,.jfif,.png,.apng,.bmp,.dib,.gif,.webp,.heic,.heif,.heicv,.heics,.tiff,.tif,.avif,.jxl,.svg,.svgz,.ico,.dng,.cr2,.cr3,.nef,.nrw,.arw,.rw2,.orf,.raf,.raw,.psd,.psb,.mp4,.m4v,.mov,.avi,.mkv,.webm,.3gp,.ogv,.mpeg,.mpg,.wmv,.flv,.hevc,.h264,.h265,.ts,.mts,.m2ts,.vob,.divx,.pdf,application/pdf"
                            ref={fileInputRef}
                            className="hidden"
                            onChange={e => {
                              if (e.target.files) {
                          ...
                          >
                            ...
                            <p className="text-[11px] text-gray-400 mt-1">Hỗ trợ: JPG, PNG, HEIC, BMP, GIF, WEBP, AVIF, RAW, PSD, MP4, MKV, MOV, AVI, HEVC, PDF,... (Tối đa 500MB)</p>
                          </div>
      ...
      {previewAttachmentsTxn && (
        <div className="modal-overlay-container">
          <div className="modal-content-container max-w-2xl">
            ...
            <div className="p-6 bg-gray-50/50 overflow-y-auto flex-1">
                ...
                {previewAttachmentsTxn.attachments?.map((file, idx) => (
                  <div key={file.id || idx} className="bg-white border border-gray-100 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-shadow group">
                    <div className="aspect-[4/3] bg-gray-100 relative overflow-hidden flex items-center justify-center">
                      ...
                      <div className={`flex flex-col items-center gap-2 ${isImageFile(file) || isVideoFile(file) ? 'hidden' : ''}`}>
                        <FileText className="w-12 h-12 text-gray-300" />
                        <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">{(file.fileType.split('/')[1] || file.fileName.split('.').pop() || 'FILE').toUpperCase()}</span>
                      </div>
                      {isVideoFile(file) && (
                        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
            ...
            <div className="border-t border-gray-100 px-6 py-4 bg-gray-50/30 flex justify-end flex-shrink-0">
              <button
                onClick={() => setPreviewAttachmentsTxn(null)}
      ...
      {showPartnerModal && (
        <PartnerFormModal
          ...
          onSuccess={async (newPartner) => {
            // Re-fetch partners to include the new one
            try {
              const pts = await partnerService.getAll();
              setPartners(pts);

              if (newPartner) {
                // Set the form data to use the new partner
                setFormData(prev => ({
                  ...prev,
                  objectType: 'PARTNER',
              ...
              console.error('Error refreshing partners after creation:', err);
            }
