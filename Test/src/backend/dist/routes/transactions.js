...
router.put('/:id', async (req, res) => {
    try {
        ...
        const user = req.user;
        const roleName = user.role.toLowerCase();
        const currentTxn = await prisma_1.default.transaction.findUnique({
            where: { id }
        });
        if (!currentTxn) {
            return res.status(404).json({ error: 'Transaction not found' });
        ...
        const { attachments, allocationPreviews, 
        // Exclude relation objects and metadata that might be present in the body
        category, project, partner, employee, businessUnit, paymentMethod, allocationRule, creator, updateReason, auditLogs, id: txnId, // Renamed to avoid shadowed variable
        ...txnData } = req.body;
...
router.put('/:id/approve', async (req, res) => {
    try {
        ...
        const currentTxn = await prisma_1.default.transaction.findUnique({ where: { id: req.params.id } });
        const transaction = await prisma_1.default.transaction.update({
            where: { id: req.params.id },
            data: {
